...
public class Turn2UrnPhase2 {
	...
	public static void main(String argv[]) {
		try {
			...
			id=Integer.valueOf(specDiagrams.get(specDiagrams.size()-1).eGet(MapPackage.Literals.UC_MMAP.getEStructuralFeature("id")).toString());
						...
						if(emptyPt.getName().contains("ConnectTimer#")) {
							...
							if(timerConnect != null) {
								if(asynch) {
									...
									emptyPt.eSet(MapPackage.Literals.PATH_NODE.getEStructuralFeature("pred"), timerPredSuccList);
									for(Object obj: emptyPtPred.getSucc()) {
										...
										if(nc.getCondition() != null && emptyPtNC.getCondition() != null) {
											if(nc.getCondition().getExpression().equals(emptyPtNC.getCondition().getExpression())) {
												emptyPtSucc.add(nc);
												timerConnect.eSet(MapPackage.Literals.PATH_NODE.getEStructuralFeature("pred"),emptyPtSucc);
									...
									if(!connectedOrFork) {
										timerConnect.eSet(MapPackage.Literals.PATH_NODE.getEStructuralFeature("pred"), timerPredList);
									}
									...
									timerConnect.eSet(MapPackage.Literals.PATH_NODE.getEStructuralFeature("succ"),connectSuccList);
							...
							if(from!="" && to!=""){
								for(Object node: map.getNodes()){
									...
									if(firstNode != null){
										...
										while(true){
											if(nd.eClass().getName().equals("OrFork") || nd.eClass().getName().equals("AndFork") || nd.eClass().getName().equals("Timer")) {
